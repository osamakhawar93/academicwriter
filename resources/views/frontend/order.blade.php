<?php 
if(isset($_GET['service'])){ $service = $_GET['service']; }else { $service = '0';}
if(isset($_GET['deadline'])){ $deadline = $_GET['deadline']; }else { $deadline = '0';}
if(isset($_GET['level'])){ $level = $_GET['level']; }else { $level = '0';}
if(isset($_GET['pages'])){ $pages = $_GET['pages']; }else { $pages = '0';}
if(isset($_GET['price'])){ $price = $_GET['price']; }else { $price = '0';}
if(isset($_GET['orginal_price'])){ $orginal_price = $_GET['orginal_price']; }else { $orginal_price = '0';}
...
<main class="content-row">
    <div class="page-title-wrapp">
				<div class="container">
					<div class="row">
						<div class="col-lg-12">
							<h3 class="page-title-01">Order Now</h3>
						</div>
					</div>
<!--					<div class="row">
						<div class="col-lg-12">
                ...
                <div class="col-sm-8">
                    ...
                    <div class="wizard-container">
                        <div class="card wizard-card" data-color="green" id="wizardProfile">
                            <form action="{{url('/saveorderform')}}" method="post" enctype="multipart/form-data" id="wizard-form">
                                <!--        You can switch " data-color="purple" "  with one of the next bright colors: "green", "orange", "red", "blue"       -->
                                    {{ csrf_field() }}
                                    <input type="hidden" name="price" id="pprice" value="{{$price}}">
                                    <input type="hidden" name="orginal_price" id="oprice" value="<?php if($orginal_price > 0){?>{{$orginal_price*2}}<?php } ?>">
                                ...
                                <div class="col-sm-12 wizard-navigation">
                                    <ul>
                                        <li id="about-tab"><a href="#about" data-toggle="tab" >1.About</a></li>
                                        <li id="account-tab"><a href="#account" data-toggle="tab" >2.Detail</a></li>
                                        <li id="address-tab"><a href="#address" data-toggle="tab">3.Payment</a></li>
                                    </ul>
                                </div>
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane" id="about">
                                        <div class="row">
                                            ...
                                            <input class="col-sm-8 reply-form__name" type="text" name="name" id="name" value="{{Session::get('name')}}" aria-required="true" aria-invalid="false" placeholder="Name *">
                                                    
                                            </div>
                                            <div class="col-sm-12 reply-form__box-01 your-email">
                                                <div class="col-sm-3"><label> Email <span class="red_star">*</span></label></div>
                                                    <input class="col-sm-8 reply-form__email" type="email" name="email" id="email" value="{{Session::get('email')}}" aria-required="true" aria-invalid="false" placeholder="Email *">
                                                    
                                            </div>
                                            <div class="col-sm-12 reply-form__box-01 your-email">
                                                <div class="col-sm-3"><label> Phone<span class="red_star">*</span></label></div>
                                                    <input class="col-sm-8 reply-form__email" type="text" name="phone" id="phone" value="{{Session::get('phone')}}" aria-required="true" aria-invalid="false" placeholder="Phone *">
                                            ...
                                            <div class="col-sm-12 reply-form__box-01 your-email" >
                                               ...
                                               <div class="col-sm-4" style="margin-bottom: 12px;"><input style="border-radius: 25px;padding-left: 32px;" id="deadline" name="date" type="text"   class="reply-form__email  datepicker" /></div>      
                                                ...
                                                <div class="col-sm-6 col-md-6 col-lg-6 reply-form__box-01 your-email" id="pricedisplay">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12 col-md-12 col-lg-12" style="margin-top: 33px">
                                                <div class="col-sm-12 reply-form__box-01 name">
                                                    ...
                                                    <input class="col-sm-8 reply-form__name" type="text" name="title" id="title" aria-required="true" aria-invalid="false" placeholder="Type assignment title">
                                                ...
                                                <div class="col-sm-12 reply-form__box-01 name">
                                                    ...
                                                    <input class="col-sm-8 reply-form__name" type="text" name="sources" id="sources" aria-required="true" aria-invalid="false" placeholder="e.g 3 Sources (1 Literary, 2 Peer Reviewed Academic Sources)">
                                                    
                                                </div>
                                                <div class="col-sm-12 reply-form__box-01 name">
                                                    <div class="col-sm-3"><label> Description</label></div>
                                            
                                                    <textarea style="margin: 0px; height: 163px;" class="col-sm-8 reply-form__name" type="text" name="description" id="description"   placeholder="Describe your task in detail or attach original file with teacher's instructions."></textarea>
                                                   
                                                </div>
                                                
                                                <div class="col-sm-12 reply-form__box-01 name">
                                                    <div class="col-sm-3"><label> Attchment <span class="red_star">*</span></label></div>
  ...
  <input type="hidden" id="amountInDollars" value="" name="amountInDollars" />
  <input type="hidden" id="stripeToken" value="" name="stripeToken" />
  <input type="hidden" id="stripeEmail" value="" name="stripeEmail" />
  <input type="hidden" id="stripePrice" value="" name="stripePrice" />
          

  <div class="centered">
      
      
      <button id="customButton" class="btn btn-success">Proceed To Payment</button>
...
var handler = StripeCheckout.configure({
  key: 'pk_test_LykBV3rvQOLhBO4uwaXrZQ6H',
  ...
  token: function(token) {
    ...
    $("#stripePrice").val($("#amountInDollars").val());
    $("#wizard-form").submit();
  }
});
...
document.getElementById('customButton').addEventListener('click', function(e) {
  ...
  handler.open({
    name: 'YourAcademicWriter',
    description: 'Payment for amount ($' + displayAmount + ')',
    currency: 'usd',
...
window.addEventListener('popstate', function() {
  handler.close();
                    ...
                    <!-- -->
                                ...
                                <div class="col-sm-11 wizard-footer">
                                    <div class="pull-right" id="appendbtn">
                                        <input type='button' id="btnnext" class='btn btn-next btn-fill btn-success btn-wd' name='next' value='Next' style="width: 120px;"/>
                                        @if(Session::has('user_id'))
                                        <input id="ordersubmit" type="button" class='btn btn-finish btn-fill btn-success btn-wd' name='finish' value='Save and pay later'/>
                                        @else
                                        <a id="ordersubmit" class='btn btn-finish btn-fill btn-success btn-wd'  name='finish' value='Save and pay later'/>Save and pay later</a>
                                        @endif
                                    </div>
                    ...
                    <div class="col-md-4" style="margin-top: 6%; border-radius: 6px; display: none" id="sideprice">
                        ...
                        <div id="rightPanelPrice" class="pricegrad" style="border-radius: 6px">  
                            <div id="price-box-2">
                                <div class="price-box-inner">
                                    <div class="pricegrad" style="border-radius: 6px">
								<p style="padding: 10px 0px 0 20px; font-size: 20px;"><b>Your Price: $<span id="ddprice" ><?php if($price == ''){echo '0.00';} else {echo $price;} ?></span></b></p>
								<div style="width:85%;">
									<p style="padding: 20px 0px 0px 22px; font-size: 20px;display:inline-block;">Was: $<span id="ttprice" style="text-decoration: line-through; text-decoration-color: red;"><?php if($price == ''){echo '0.00';} else {echo $price*2;} ?></span></p>
									<p style="padding: 20px 0px 0px 25px; font-size: 20px;display:inline-block;">Now: $<span id="dd2price" ><?php if($price == ''){echo '0.00';} else {echo $price;} ?></span></p>
								</div>
								<p style="padding: 0 20px 0 20px; font-size: 20px;"><b>You Save: $<span id="dd3price" ><?php if($price == ''){echo '0.00';} else {echo $price;} ?></span></b></p>
								<p style="padding: 0 20px 25px 20px; font-size: 20px;">Offer Ends: <i class="fa fa-clock-o"></i> 7 Days</p>
                            ...
                            <div class="check-btn" style="padding-left: 55px;">
                                <input type="button" id="btncheckout"  class="btn btn-primary" value="REVIEW &amp; CHECKOUT">
                            </div>
                          ...
                          <div class="payment-section">
                              ...
                              <a href="" onclick="isOrderFormValid();" style="padding: 30px"><img src="img/payment-option.png" alt="" title=""></a>
                           </div>
                         </div>
...
$.ajaxSetup({
    ...
    $('#btnnext').click(function(){
        $("#ttprice").text($("#tprice").text());
        $("#ddprice").text($("#dprice").text());
        $("#dd2price").text($("#d2price").text());
        $("#dd3price").text($("#d3price").text());
        //alert($("#tprice").text());
        $('#pricedisplay').css('display','none');
        $('#sideprice').css('display','block');
...
$(document).ready(function(){
    ...
    if('{{$price}}' != ''){
        ...
        document.getElementById("pprice").value = '{{$price}}';
          document.getElementById("amountInDollars").value = '{{$price}}';
   }
			
        $('#deadline,#pages,#level,#hours').on('change', function() {	
				...
				document.getElementById("amountInDollars").value = dtotal;
				
                                $("#ttprice").text(ttotal);
                                $("#ddprice").text(dtotal);
                                $("#dd2price").text(dtotal);
                                $("#dd3price").text(dtotal);
                                if(dl > '-1' && dl > 0){
...
var date = '{{$deadline}}';
if(date != '' ){
var a = someDate.setDate(someDate.getDate() + parseInt(date));
...
$('#ordersubmit').on('click',function(){
    if('{{Session::has("user")}}'){
         ...
         var error = checkforFormErrorsInTab2();
...
function checkforFormErrorsInTab2(){
    
    var error = false;
    
    var title = $("#title").val();
    var citationStyle = $('#citation').val();
    var subject  = $("#citation").val();
    var noOfSources = $("#sources").val();
    ...
    if(title == undefined || title == null || title == ""){
     ...
     if(citationStyle == undefined || citationStyle == null || citationStyle == ""){
        var error = true;
    }
    ...
    if(noOfSources == undefined || noOfSources == null || noOfSources==""){
        var error = true;
    }
